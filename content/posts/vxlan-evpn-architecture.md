...
2. **Retain EVPN routes without local VNIs.** A transit spine has no tenant VRFs or import RTs. On NX-OS, `retain route-target all` under `address-family l2vpn evpn` allows it to retain and advertise EVPN routes that have no locally importable RT. Note that Model A never needs this knob even though its RR spines are equally VNI-less: RFC 4364's RT-filtering rule is automatically suspended on a route reflector the moment `route-reflector-client` is configured, and it is precisely that exemption that an eBGP transit tier — having no RR role — lacks. [Cisco Nexus 9000 eBGP EVPN procedure](https://www.cisco.com/c/en/us/td/docs/dcn/nx-os/nexus9000/105x/configuration/vxlan/cisco-nexus-9000-series-nx-os-vxlan-configuration-guide-release-105x/m_configuring_vxlan_bgp_evpn.html)
...
For the full control-plane, forwarding, and failure walkthrough, see [Appendix A: EVPN Type 1 and Type 4 multihoming reference](#appendix-a-evpn-type-1-and-type-4-multihoming-reference).
...
The interactive walkthrough below generalizes this walk. It uses its own small addressing plan (VTEPs `192.0.2.x`, VNIs 1010/1020/50001) rather than the pod plan above, but the mechanics are the point: pick Server A's destination, toggle between the two section 10.2 IRB models, and watch how the three packet stages change — routed through the VRF's L3 VNI (symmetric) or straight into the destination VLAN's L2 VNI (asymmetric), bridged in an L2 VNI, bridged or routed to a second site through either one EVPN domain (a single leaf-to-leaf tunnel, the section 12.2 multi-pod model) or separate domains stitched across three tunnels at border gateways (section 13's Multi-Site machinery, Arista's multi-domain EVPN gateway), routed via a leaked route into another VRF's L3 VNI, or dropped at ingress when no route was imported. The two different-VLAN remote-site cases make an important distinction visible: symmetric IRB uses tenant `L3VNI_A 50001`, whereas asymmetric IRB uses destination `L2VNI_20 1020`; merely crossing from VLAN 10 to VLAN 20 does not always put VNI 1020 on the wire. In each multi-domain case, stage 2 expands into three separate packet snapshots — arriving at BGW-A, crossing the DCI, and leaving BGW-B — so the changed outer headers at both tunnel boundaries are explicit. Note what stays constant in every forwarded case: an outer header whose UDP source port carries the ECMP entropy — and a single leaf-to-leaf encapsulation everywhere except the multi-domain cases, whose three-tunnel stitching is exactly the section 14 walk.

{{< embed src="/posts/vxlan-evpn-architecture/vxlan-packet-walk.html" title="VXLAN EVPN packet walk, case by case" height="900" wide="true" >}}
...
If only VLAN 110 fails while the physical Ethernet segment remains up, the per-EVI Type 1 withdrawal gives the narrower, service-specific signal. Implementations that support the RFC 8584 AC-DF capability can also use Type 1 service availability to remove an ineligible leaf from that service's DF candidate list. [RFC 8584, Section 4](https://www.rfc-editor.org/rfc/rfc8584.html#section-4)
...
![Type 4 Ethernet Segment route exchange between PE1 and PE2, with Remote PE3 excluded by the ES-Import Route Target](/posts/vxlan-evpn-architecture/evpn-mh-step1-type4-discovery.svg)
...
![Type 1 per-ES and per-EVI reachability from PE1 and PE2, combined with the Type 2 route for MAC M1 at Remote PE3](/posts/vxlan-evpn-architecture/evpn-mh-step2-type1-aliasing.svg)
...
Suppose only `PE1` learns endpoint MAC `M1` and advertises its Type 2 route with `ESI 01`. `Remote PE3` combines that Type 2 route with the Type 1 per-EVI routes from both PEs and expands the next-hop set from `{PE1}` to `{PE1, PE2}`. This is aliasing: Type 2 identifies the MAC, while Type 1 identifies the additional path through the same Ethernet segment. [RFC 7432, Section 8.4](https://www.rfc-editor.org/rfc/rfc7432.html#section-8.4)

In a VXLAN service, describe this control-plane association using the **EVI/L2 VNI**, not only the locally significant VLAN number. RFC 8365 maps the VNI into the EVPN route's label field. [RFC 8365, Section 5.1.3](https://www.rfc-editor.org/rfc/rfc8365.html#section-5.1.3)

#### A.6.3 Withdraw the failed attachment

![Type 1 fast withdrawal after the complete CE-to-PE1 attachment fails, leaving PE2 as the only next hop and designated forwarder](/posts/vxlan-evpn-architecture/evpn-mh-step3-fast-withdrawal.svg)
